 ...
 */
export function resolveAllUnderline(events: Event[], context: TokenizeContext): Event[] {
  let index = -1;
  ...
  while (++index < events.length) {
    ...
    if (canClose(events[index], 'underline')
      ...
      while (open--) {
        ...
        // The token must have at least 2 markers (`++`) because
        // underline matches `++<text>++` sequences.
        if (canOpen(events[open], 'underline')
          && events[open][1].end.offset - events[open][1].start.offset > 1
        ) {
          const nextIndex = addEvents(events, open, index, context, 'underline', 2);
          index = nextIndex;
          break;
        }
      }
